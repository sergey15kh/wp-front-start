...
<img src="https://i.ibb.co/7zgXC4G/1.png" alt="1" border="0">
...
9. Обязательно верстку проверяем на валидность через сервис <a href="https://validator.w3.org/">validator</a>

10. В проекте должен быть 1 файл стилей CSS, и один файл скриптов JS. Стили подключаются в хедере, а скрипты в футере. Это необходимо для улучшения скорости загрузки сайта по Google Page Speed.

11. Если требуется подключить сторонние библиотеки, то их JS должен подключаться в футере в наш основной единый файл, а стили в хедере, также в наш основной файл. Кроме FontsAwesome, их можно отдельно.

12. Не используем Bootstrap, занимаем много места и задерживает загрузку страницы своими CSS, JS.

13. Меню в футере и в хедере также редактируются с админки, если нужно добавить классы на пункты меню - то в этом случае нужно их добавить на `<li>` теги.

14. Текстовые строки, блоки в футере, хедере также редактируются с админки и к ним применяются те же правила, что и к основному контенту. 

15. Если нужно поставить изображение на фон - то нужно его прописать через атрибут `style=””` в теге, а не в файле стилей CSS.
